...
lcase_sse_simple(char *restrict dst, const char *restrict src, size_t len)
{
    	...
    	__m128i case_bit = _mm_set1_epi8(CASE_BIT);	// load case bit into register

    	// process input data in 128-bit chunks
    	for (size_t i = 0; i < len; i += 16) {
        	__m128i input = LOAD_SI128((__m128i *)(src + i));	// load 16 characters from src into SIMD register

        	__m128i result = _mm_or_si128(input, case_bit);		// perform bitwise OR operation between input and case_bit

        	STORE_SI128((__m128i *)(dst + i), result);	// store result in dst
    }
}
...
lcase_sse_cond(char *restrict dst, const char *restrict src, size_t len)
{
   	...
   	// load upper and lower bounds of character range ('A' - 1 and 'Z' + 1) into registers
    	...
    	for (size_t i = 0; i < len; i += 16) {
        	...
        	// compare character with upper and lower bounds
        	__m128i cmp_lower = _mm_cmpgt_epi8(input, lower_bound);
        	__m128i cmp_upper = _mm_cmpgt_epi8(upper_bound, input);

        	__m128i mask = _mm_and_si128(cmp_lower, cmp_upper);	// bitwise AND operation to get mask for characters within range

        	// set lowercase bit for characters within range
        	__m128i case_bit = _mm_set1_epi8(CASE_BIT);
        	__m128i result = _mm_or_si128(input, _mm_and_si128(mask, case_bit));
