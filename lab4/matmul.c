...
matmul_sse()
{
    ...
    for (i = 0; i < SIZE; i++) {
        for (k = 0; k < SIZE; k++) {
            __m128 a_row = _mm_set1_ps(mat_a[i][k]); // Load the entire row of mat_a[i]

            for (j = 0; j < SIZE; j += 4) {
                __m128 b_col = _mm_load_ps(&mat_b[k][j]); // Load 4 elements from mat_b[k][j]

                __m128 result = _mm_mul_ps(a_row, b_col); // Multiply row with column

                // Accumulate the result into the corresponding elements of mat_c[i][j:j+3]
                __m128 sum = _mm_loadu_ps(&mat_c[i][j]);
                sum = _mm_add_ps(sum, result);
                _mm_storeu_ps(&mat_c[i][j], sum);
            }
        }
    }
}
