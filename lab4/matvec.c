...
matvec_sse()
{
   	...
   	// initialize SSE registers 
    	...
    	__m128 a_row, b_vec, result;

    	for (int i = 0; i < SIZE; i++) {
        	sum = _mm_setzero_ps();
        	for (int j = 0; j < SIZE; j += 4) {
            		a_row = _mm_load_ps(&mat_a[MINDEX(i, j)]);	// load elements of mat_a into single SSE register

            		b_vec = _mm_load_ps(&vec_b[j]);		// load elements of vec_b into single SSE register

            		result = _mm_mul_ps(a_row, b_vec);	// multiply mat_a and vec_b

            		sum = _mm_add_ps(sum, result);		// accumulate results
        	}

        	// horizontal add to get sum of all elements in sum register
        	sum = _mm_hadd_ps(sum, sum);
        	sum = _mm_hadd_ps(sum, sum);

        	// extract result from SSE register and store in vec_c
        	float result_array[4];
        	_mm_store_ps(result_array, sum);
        	vec_c[i] = result_array[0];
    	}
}
